...
const Halls = () => {
  ...
  const saveHall = async () => {
    ...
    if (!formData.hallName.trim() || totalRows < 1 || totalCols < 1) {
      alert('Vui long nhap ten phong, so hang va so cot hop le.');
      return;
    }
  ...
  return (
    <div className="flex flex-col gap-6">
      <div className="flex items-center justify-between">
        <div>
          <h2 className="font-headline-sm text-headline-sm text-text-primary">Quan ly phong chieu</h2>
          <p className="mt-1 text-sm text-text-muted">Quan ly phong chieu va cau hinh so do ghe.</p>
        </div>
      ...
      <div className="flex flex-wrap items-end gap-4 rounded-xl border border-border-default bg-surface p-4 shadow-sm">
        <label className="block w-64">
          <span className="mb-1.5 block text-xs font-semibold uppercase tracking-wider text-text-secondary">Chon rap</span>
        ...
        <label className="block w-64">
          <span className="mb-1.5 block text-xs font-semibold uppercase tracking-wider text-text-secondary">Tim phong</span>
          <input value={searchQuery} onChange={(event) => setSearchQuery(event.target.value)} disabled={!selectedCinemaId} placeholder="Tim theo ten..." className="w-full rounded-lg border border-border-default bg-surface-container-low px-3 py-2 text-sm outline-none focus:border-primary disabled:opacity-50" />
        </label>
        <label className="block w-48">
          <span className="mb-1.5 block text-xs font-semibold uppercase tracking-wider text-text-secondary">Trang thai</span>
          <select value={statusFilter} onChange={(event) => setStatusFilter(event.target.value)} disabled={!selectedCinemaId} className="w-full rounded-lg border border-border-default bg-surface-container-low px-3 py-2 text-sm outline-none focus:border-primary disabled:opacity-50">
            ...
            <option value="CONFIGURED">Da cau hinh</option>
      ...
      {selectedCinemaId && (
        <div className="grid grid-cols-2 gap-4 md:grid-cols-4 lg:grid-cols-5">
          <div className="rounded-xl border border-border-default bg-surface p-4 shadow-sm"><p className="text-xs font-semibold uppercase tracking-wider text-text-secondary">Tong phong</p><p className="mt-2 text-2xl font-bold text-text-primary">{summary.total}</p></div>
          <div className="rounded-xl border border-border-default bg-surface p-4 shadow-sm"><p className="text-xs font-semibold uppercase tracking-wider text-text-secondary">Tong ghe</p><p className="mt-2 text-2xl font-bold text-text-primary">{summary.seats}</p></div>
          <div className="rounded-xl border border-border-default bg-surface p-4 shadow-sm"><p className="text-xs font-semibold uppercase tracking-wider text-text-secondary">Phong lon nhat</p><p className="mt-2 truncate text-lg font-bold text-text-primary">{summary.largest}</p></div>
          <div className="rounded-xl border border-border-default bg-surface p-4 shadow-sm"><p className="text-xs font-semibold uppercase tracking-wider text-text-secondary">Chua co so do</p><p className={`mt-2 text-2xl font-bold ${summary.withoutLayout > 0 ? 'text-warning' : 'text-text-primary'}`}>{summary.withoutLayout}</p></div>
          <div className="hidden rounded-xl border border-border-default bg-surface p-4 shadow-sm lg:block"><p className="text-xs font-semibold uppercase tracking-wider text-text-secondary">Ghe TB/phong</p><p className="mt-2 text-2xl font-bold text-text-primary">{summary.avgSeats}</p></div>
        </div>
      )}
      ...
      <div className="overflow-hidden rounded-xl border border-border-default bg-surface shadow-sm">
        {!selectedCinemaId ? (
          <div className="flex flex-col items-center justify-center py-20 text-text-muted">
            ...
            <p>Chon rap de quan ly phong chieu.</p>
          </div>
        ) : (
          <div className="overflow-x-auto">
            <table className="w-full text-left text-sm text-text-primary">
              <thead className="border-b border-border-default bg-surface-container-low">
                <tr>
                  <th className="px-4 py-3 font-semibold">Ten phong</th>
                  <th className="px-4 py-3 font-semibold">So hang</th>
                  <th className="px-4 py-3 font-semibold">So cot</th>
                  <th className="px-4 py-3 font-semibold">Tong ghe</th>
                  <th className="px-4 py-3 font-semibold">So do</th>
                  <th className="px-4 py-3 text-right font-semibold">Thao tac</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-border-default">
                {loading ? (
                  <tr><td colSpan="6" className="py-8 text-center text-text-muted">Dang tai...</td></tr>
                ) : filteredHalls.length === 0 ? (
                  <tr><td colSpan="6" className="py-8 text-center text-text-muted">Khong co phong phu hop.</td></tr>
                ) : (
                      ...
                      <tr key={hall.HallID} className="transition-colors hover:bg-surface-variant/30">
                        ...
                        <td className="px-4 py-3 text-right">
                          <div className="flex justify-end gap-2">
                            <button onClick={() => openModal(hall)} className="inline-flex h-8 w-8 items-center justify-center rounded-lg border border-border-default text-text-secondary transition-colors hover:bg-surface-variant hover:text-text-primary" title="Sua">
                              <span className="material-symbols-outlined text-[18px]">edit</span>
                            </button>
                            <button onClick={() => navigate('/seat-layout', { state: { cinemaId: selectedCinemaId, hallId: hall.HallID } })} className="inline-flex h-8 w-8 items-center justify-center rounded-lg border border-primary/50 text-primary transition-colors hover:bg-primary-soft" title="Cau hinh ghe">
                              <span className="material-symbols-outlined text-[18px]">grid_view</span>
                            </button>
                            <button onClick={() => deleteHall(hall.HallID)} className="inline-flex h-8 w-8 items-center justify-center rounded-lg border border-error/50 text-error transition-colors hover:bg-error/10" title="Xoa">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4 backdrop-blur-sm">
          <div className="w-full max-w-[520px] overflow-hidden rounded-xl border border-border-default bg-surface shadow-2xl">
            <div className="flex items-center justify-between border-b border-border-default bg-surface-container-low px-6 py-4">
              <h3 className="text-lg font-bold text-text-primary">{editingHallId ? 'Sua phong' : 'Them phong moi'}</h3>
            ...
            <div className="space-y-4 p-6">
              <label className="block">
                <span className="mb-1.5 block text-sm font-semibold text-text-secondary">Ten phong</span>
              ...
              <div className="grid grid-cols-2 gap-4">
                <label className="block">
                  <span className="mb-1.5 block text-sm font-semibold text-text-secondary">So hang</span>
                ...
                <label className="block">
                  <span className="mb-1.5 block text-sm font-semibold text-text-secondary">So cot</span>
              ...
              {Number(formData.totalRows) > 0 && Number(formData.totalCols) > 0 && (
                <p className="text-xs italic text-text-muted">Phong co toi da <strong className="text-text-primary">{Number(formData.totalRows) * Number(formData.totalCols)}</strong> vi tri ghe.</p>
              )}
            </div>
            <div className="flex justify-end gap-3 border-t border-border-default bg-surface-container-low px-6 py-4">
              <button onClick={closeModal} className="rounded-lg border border-border-default px-4 py-2 text-sm font-semibold text-text-primary transition-colors hover:bg-surface-variant">Huy</button>
