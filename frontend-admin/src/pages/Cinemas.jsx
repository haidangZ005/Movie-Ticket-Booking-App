...
const Cinemas = () => {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      alert(err.message || 'Không thêm được rạp.');
    }
  };
  ...
  return (
    <>
      <div className="mb-6 flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
        <div>
          <h2 className="font-headline-sm text-headline-sm text-text-primary">Rạp chiếu & Suất chiếu</h2>
          <p className="mt-1 text-sm text-text-muted">Quản lý cụm rạp, phòng chiếu và lịch chiếu theo database.</p>
        </div>
        <button
      ...
      <div className="grid grid-cols-1 gap-6 xl:grid-cols-3">
        <div className="overflow-hidden rounded-xl border border-border-default bg-surface shadow-sm xl:col-span-2">
          <div className="border-b border-border-default px-5 py-4">
            <h3 className="text-sm font-bold uppercase tracking-wider text-text-secondary">Cụm rạp</h3>
          </div>
          <div className="overflow-x-auto">
            <table className="w-full min-w-[760px] text-left">
              <thead className="border-b border-border-default bg-surface-container-low">
                <tr>
                  {['Rạp', 'Địa chỉ', 'Quận/Huyện', 'Thành phố', 'Tọa độ', 'Trạng thái'].map((heading) => (
              ...
              <tbody className="divide-y divide-border-default text-[13px]">
                {loading ? (
                  <tr><td className="px-4 py-6 text-center text-text-muted" colSpan="6">Đang tải...</td></tr>
                ) : cinemas.length === 0 ? (
                  <tr><td className="px-4 py-6 text-center text-text-muted" colSpan="6">Chưa có rạp nào. Bấm Thêm rạp để tạo rạp đầu tiên.</td></tr>
                ) : (
                  cinemas.map((cinema) => (
                    <tr key={cinema.CinemaID} className="transition-colors hover:bg-surface-container-low">
                      ...
                      <td className="px-4 py-3"><span className={`badge ${cinema.IsActive ? 'success' : 'gray'}`}>{cinema.IsActive ? 'Hoạt động' : 'Ngừng hoạt động'}</span></td>
                    </tr>
                  ))
                )}
              </tbody>
        ...
        <div className="rounded-xl border border-border-default bg-surface p-card-padding shadow-sm">
          <h3 className="mb-4 text-sm font-bold uppercase tracking-wider text-text-secondary">Cấu hình rạp</h3>
          <div className="space-y-3 text-sm text-text-secondary">
            <div className="rounded-lg border border-border-default bg-surface-container-low p-4">
              <div className="font-semibold text-text-primary">Thành phố</div>
              <div className="mt-1 text-text-muted">{cities.length} bản ghi thành phố</div>
            </div>
            <div className="rounded-lg border border-border-default bg-surface-container-low p-4">
              <div className="font-semibold text-text-primary">Rạp chiếu</div>
              <div className="mt-1 text-text-muted">{cinemas.length} bản ghi rạp đang hoạt động</div>
            </div>
          </div>
        </div>
      </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4">
          <div className="max-h-[90vh] w-full max-w-2xl overflow-y-auto rounded-xl border border-border-default bg-surface p-6 shadow-xl">
            <div className="mb-5 flex items-center justify-between">
              <h3 className="text-lg font-semibold text-text-primary">Thêm rạp</h3>
            ...
            <form onSubmit={handleSubmit} className="grid grid-cols-1 gap-4 md:grid-cols-2">
              <label className="block md:col-span-2">
                <span className="mb-1.5 block text-sm font-medium text-text-secondary">Tên rạp</span>
              ...
              <label className="block">
                <span className="mb-1.5 block text-sm font-medium text-text-secondary">Thành phố có sẵn</span>
                <select name="cityId" value={formData.cityId} onChange={handleChange} className="w-full rounded-lg border border-border-default bg-surface-container-low px-3 py-2 outline-none focus:border-primary">
                  <option value="">Tạo thành phố mới</option>
                  {cities.map((city) => (
              ...
              <label className="block">
                <span className="mb-1.5 block text-sm font-medium text-text-secondary">Tên thành phố mới</span>
              ...
              <label className="block">
                <span className="mb-1.5 block text-sm font-medium text-text-secondary">Quận/Huyện</span>
              ...
              <label className="block">
                <span className="mb-1.5 block text-sm font-medium text-text-secondary">Địa chỉ</span>
              ...
              <div className="flex justify-end gap-3 md:col-span-2">
                ...
                <button type="submit" className="rounded-lg bg-primary px-4 py-2 text-sm font-semibold text-inverse-on-surface hover:bg-accent-hover">Lưu rạp</button>
              </div>
            </form>
          </div>
        </div>
