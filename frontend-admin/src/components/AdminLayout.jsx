...
const navItems = [
  ...
  { to: '/movies', label: 'Phim', icon: 'movie' },
  { to: '/cinemas', label: 'Rạp chiếu', icon: 'theaters' },
  { to: '/showtimes', label: 'Suất chiếu', icon: 'event' },
  { to: '/products', label: 'Sản phẩm', icon: 'fastfood' },
  { to: '/vouchers', label: 'Voucher', icon: 'local_offer' },
  { to: '/bookings', label: 'Đặt vé', icon: 'confirmation_number' },
  { to: '/customers', label: 'Khách hàng', icon: 'group' },
  { to: '/transactions', label: 'Giao dịch', icon: 'receipt_long' },
    ...
    <div className="min-h-screen overflow-x-hidden bg-page-bg text-on-surface antialiased">
      <aside className="fixed left-0 top-0 z-40 hidden h-full w-[240px] flex-col border-r border-border-default bg-surface md:flex">
        ...
        <div className="px-6 py-4">
          <p className="mb-4 text-xs font-semibold uppercase tracking-wider text-text-muted">Quản trị rạp phim</p>
        </div>
      ...
      <header className="fixed right-0 top-0 z-30 hidden h-[64px] w-[calc(100%-240px)] items-center justify-between border-b border-border-default bg-surface px-container-padding md:flex">
        <div className="relative w-64">
          ...
          <input className="w-full rounded-lg border border-border-default bg-surface-container-low py-2 pl-10 pr-4 text-sm text-text-primary outline-none transition-shadow placeholder:text-text-muted focus:ring-2 focus:ring-primary/30" placeholder="Tìm kiếm" type="text" />
        </div>

        <div className="flex items-center gap-4">
          <button className="relative rounded-full p-2 text-text-secondary transition-colors hover:bg-surface-container-low hover:text-primary" title="Thông báo">
          ...
          <div className="flex items-center gap-3 border-l border-border-default pl-4">
            ...
            <div className="flex flex-col">
              <span className="text-sm font-semibold text-text-primary">Quản trị viên</span>
